...
import { isFutureDate, todayLocal } from './dates';
...
export function migrate(raw: unknown): AppState {
  ...
  const furniture = (Array.isArray(state.furniture) ? state.furniture : [])
    .flatMap((raw): AppState['furniture'] => {
      ...
      // The repaired day is the LOCAL day, because that is the day the piece
      // was added on as far as the person standing in front of it is concerned,
      // and because WardrobeContext writes todayLocal() when it makes a new
      // piece — a UTC label here would stamp a wardrobe furnished in Kiritimati
      // before 14:00 with yesterday, and one furnished in Niue after 13:00 with
      // tomorrow, in a document where every other date is local.
      if (typeof f.dateAdded !== 'string') f.dateAdded = todayLocal();
      return [f];
    });

  // v8: how this document holds its photographs. The web app writes base64
  // data URIs into `imageUrl`; the native app keeps the files on disk and
  // writes paths relative to its document directory into the same field. Both
  // are strings, so nothing downstream could tell them apart and the mismatch
  // would have been silent — forty photographs resolving to nothing, with no
  // error anywhere. Every document written before this field existed came out
  // of the web app, so an absent declaration is 'inline'. A value we do not
  // recognise is repaired to 'inline' rather than trusted, because the two
  // wrong readings are not equally bad: a data URI read as a path still shows
  // the photograph, while a path read as a data URI blanks every one at once.
  const photoEncoding: AppState['photoEncoding'] = state.photoEncoding === 'file' ? 'file' : 'inline';
