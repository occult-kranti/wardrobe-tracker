...
/** Where a photograph actually lives — see AppState.photoEncoding. */
export type PhotoEncoding = 'inline' | 'file';
...
export interface AppState {
  ...
  furniture: Furniture[];
  /**
   * How THIS document holds its photographs. The web app writes base64 data
   * URIs into every `imageUrl`; the native app keeps the files on disk under
   * FileSystem.documentDirectory and writes paths relative to it into the
   * same field. Both are strings, so nothing downstream could ever tell them
   * apart — which is why the document has to say. An export is always
...
// physically lives. v7: the new furniture forms, carved treatments, packed-away
// slots. v8: photoEncoding — the document declares whether its photographs are
// inline data URIs or files on disk. Migration seeds each on older exports;
// scripts/test-migrate.mjs holds a case for every one, written before the field
// existed.
